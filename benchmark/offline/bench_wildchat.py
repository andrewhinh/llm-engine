...
WILDCHAT_BASE_URL = (
    "https://huggingface.co/datasets/allenai/WildChat-4.8M/resolve/main/data/"
)
...
def run_offline_benchmark():
    ...
    sampling_params = [
        SamplingParams(
            temperature=0.6, ignore_eos=IGNORE_EOS, max_tokens=MAX_OUTPUT_LEN
        )
        for _ in range(num_reqs)
    ]

    llm = LLM(resolved_path)
    ...
    print(
        f"Output budget: {total_output_budget}tok, "
        f"Actual output: {total_output_tokens}tok"
    )
    print(
