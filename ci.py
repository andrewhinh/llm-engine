...
AMPERE_GPU_TYPE = "a100"
HOPPER_GPU_TYPE = "h200"
BLACKWELL_GPU_TYPE = "b200"

CI_IMAGE = (
    get_runtime_image(gpu_type=AMPERE_GPU_TYPE, extra_packages=("apache-tvm-ffi",))
    .add_local_dir(
...
@app.function(
    gpu=f"{AMPERE_GPU_TYPE}:2",
    image=CI_IMAGE,
)
async def pytest_ampere(target: str, pytest_args: str):
    import asyncio
    import shlex
...
@app.function(
    gpu=f"{HOPPER_GPU_TYPE}:2",
    image=CI_IMAGE,
)
async def pytest_hopper(target: str, pytest_args: str):
    import asyncio
    import shlex
...
async def main(target: str = "tests", pytest_args: str = "-s -vv"):
    await asyncio.gather(
        pytest_ampere.remote.aio(target, pytest_args),
        pytest_hopper.remote.aio(target, pytest_args),
