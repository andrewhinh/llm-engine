...
from llmeng.utils import Registry, init_logger
...
def create_attention_backend(
    ...
    if "," in backend:
        ...
        if p_backend != d_backend:
            logger.info(
                f"Using hybrid attention backend: prefill={p_backend}, decode={d_backend}"
            )
        ...
        logger.warning(
            f"P/D attention backends are the same: {backend}, using single backend."
        )
