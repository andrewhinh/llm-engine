...
from llmeng.utils import UNSET, Unset, init_logger
...
async def benchmark_one_batch(
    ...
    *,
    ...
    if input_lengths is None:
        input_lengths = cast(List[int | None], [None] * len(prompts))
...
def process_benchmark_results(
    raw_data: List[RawResult], tokenizer: Any
) -> BenchmarkResult: ...
...
def process_benchmark_results(
    ...
    def _print_stats(times: List[float], scale: float = 1.0) -> Tuple[float, ...]:
        assert len(times) > 0
        return (
            scale * sum(times) / len(times),
            scale * times[int(len(times) * 0.5)],
            scale * times[int(len(times) * 0.9)],
            scale * times[int(len(times) * 0.99)],
            scale * max(times),
        )
    ...
    logger.info(
        f"Throughput: {_fmt(num_tokens / dur)} token/s, {_fmt(num_requests / dur)} req/s"
    )
