...
void custom_all_gather(fptr_t _fa, torch::Tensor& inp, torch::Tensor& out,
  ...
  switch (out.scalar_type()) {
    case at::ScalarType::Float: {
      fa->all_gather<float>(stream, reinterpret_cast<float*>(reg_buffer),
                           reinterpret_cast<float*>(out.data_ptr()),
                           out.numel());
    ...
    case at::ScalarType::Half: {
      fa->all_gather<half>(stream, reinterpret_cast<half*>(reg_buffer),
                          reinterpret_cast<half*>(out.data_ptr()), out.numel());
    ...
    default:
      throw std::runtime_error(
          "custom all gather only supports float32, float16 and bfloat16");
