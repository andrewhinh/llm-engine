...
from llmeng.utils import nvtx_annotate
...
class BaseTransformerModel(BaseOP):
    def __init__(
        ...
        self.layers = OPList(
            [decoder_layer(config, layer_id) for layer_id in range(config.num_layers)]  # type: ignore[call-arg]
        )
...
class CausalLMModel(BaseLLMModel):
    def __init__(self, config: ModelConfig, model_cls: Type[BaseOP]) -> None:
        self.model = model_cls(config)  # type: ignore[call-arg]
        self.lm_head = ParallelLMHead(
            num_embeddings=config.vocab_size,
            embedding_dim=config.hidden_size,
            tie_word_embeddings=config.tie_word_embeddings,
            tied_embedding=getattr(self.model, "embed_tokens", None)
