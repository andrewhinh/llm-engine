...
from llmeng.message.utils import unwrap_batch_msg, wrap_single_or_batch_msg
from llmeng.utils import ZmqPullQueue, ZmqPushQueue, init_logger, load_tokenizer
...
def tokenize_worker(
    *,
    ...
    send_backend = ZmqPushQueue(
        backend_addr, create=False, encoder=BaseBackendMsg.encoder
    )
    send_frontend = ZmqPushQueue(
        frontend_addr, create=False, encoder=BaseFrontendMsg.encoder
    )
    ...
    try:
        while True:
            ...
            assert len(detokenize_msg) + len(tokenize_msg) + len(abort_msg) == len(
                pending_msg
            )
